...
def mymem0ry_memory_instructions() -> str:
    ...
    return (
        ...
        "\n"
        "## Session Start\n"
        "ALWAYS call get_context(cwd=<cwd>) first.\n"
        "\n"
        "## Memory Stack\n"
        "Use these complementary systems together:\n"
        "  - myMem0ry → decisions, facts, patterns, handoffs, conversations.\n"
        "  - Serena → code navigation, refactoring, symbol editing (IDE via LSP).\n"
        "  - codebase-memory-mcp → architecture, call graphs, impact analysis.\n"
        "\n"
        "Rule: WHY/WHAT DECIDED → myMem0ry. EDIT/NAVIGATE SYMBOLS → Serena.\n"
        "ARCHITECTURE/CALL GRAPH/IMPACT → codebase-memory-mcp.\n"
        "\n"
        "## During Session\n"
        "Save worth-remembering items with save_memory (returns an id):\n"
        "   - Decisions → scope='project', cwd=<cwd>, memory_type='decision'\n"
        "   - Facts → memory_type='fact'\n"
        "   - Patterns → memory_type='pattern'\n"
        "Pin important ones with memory_pin(<id>).\n"
        "\n"
        "## Fact Evolution\n"
        "When information changes:\n"
        "   1. search_memory() to find outdated facts\n"
        "   2. evolve_fact(old_ids=[...], evolved_content='...', rationale='...')\n"
        "\n"
        "## Session End\n"
        "Call memory_handoff_begin(summary='...', cwd=<cwd>). Lifecycle hooks archive\n"
        "conversations automatically; explicit summaries are still better.\n"
        "\n"
        "## Search\n"
        "   - search_memory(query='...', cwd=<cwd>) → curated memories.\n"
        "   - search_conversations(query='...') → archived transcripts.\n"
