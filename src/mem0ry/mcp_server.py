...
def mymem0ry_memory_instructions() -> str:
    ...
    return (
        ...
        "\n"
        "## Session Start\n"
        "ALWAYS call get_context(cwd=<cwd>) at the start of every session — do not\n"
        "assume the hook injection was sufficient. The hook provides a summary, but\n"
        "get_context() gives you the full, ranked context with pinned memories first.\n"
        "\n"
        "## Proactive Context Retrieval\n"
        "Before executing workflows (release, deploy, migrate, etc.), search for\n"
        "related memories by tags or keywords. Pinned memories are important —\n"
        "prioritize them. Example: if user asks for 'release', search for memories\n"
        "tagged 'release', 'deploy', 'workflow'.\n"
        "\n"
        "## During Session\n"
