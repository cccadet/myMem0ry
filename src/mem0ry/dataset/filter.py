...
def apply_quality_filters(
    ...
    return [ex for ex in examples if _has_quality_content(ex, min_turns, min_length)]


def _has_quality_content(example: dict, min_turns: int, min_length: int) -> bool:
    ...
    if len(roles) < min_turns:
        return False
    trimmed = [
        c.strip() for c in (m.get("content", "") for m in messages) if c and c.strip()
    ]
    if not trimmed:
        return False
