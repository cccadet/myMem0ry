#!/usr/bin/env bash
# Claude Code generic hook — fires on PreToolUse, PostToolUse, Notification, etc.
# Sends observation to myMem0ry server via HTTP.
set -euo pipefail

CWD="$(pwd)"
EVENT="${1:-other}"
SESSION_ID="${MYMEM0RY_SESSION_ID:-$(date +%s | md5sum | head -c 8)}"
SERVER="${MEM0RY_SERVER_URL:-http://127.0.0.1:49374}"

KIND="other"
case "$EVENT" in
    PreToolUse|PostToolUse) KIND="post-tool-use" ;;
    Notification) KIND="other" ;;
    *) KIND="other" ;;
esac

CONTENT=""
if [[ ! -t 0 ]]; then
    CONTENT="$(head -c 4096)"
fi

if [[ -n "$CONTENT" ]]; then
    CONTENT_ESCAPED=$(printf '%s' "$CONTENT" | python3 -c 'import sys,json; print(json.dumps(sys.stdin.read()))' 2>/dev/null || echo '""')
    curl -sf --max-time 0.2 -X POST "${SERVER}/hook" \
      -H "Content-Type: application/json" \
      -d "{\"kind\":\"${KIND}\",\"session_id\":\"${SESSION_ID}\",\"cwd\":\"${CWD}\",\"agent\":\"claude-code\",\"body\":${CONTENT_ESCAPED}}" \
      2>/dev/null || true
fi
